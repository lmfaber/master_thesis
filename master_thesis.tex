...
\subsection{Basic graph theory}    	
	...
	%A cluster would then be defined as a connected sub-graph \citep{graph-based:07}.
	%One widely used algorithm is the Markov Cluster Chain algorithm (\mcl) \citep{mcl:00}.
...
\subsubsection*{karma}
	...
	%https://academic.oup.com/bioinformatics/article/32/12/i192/2288985
	% Rather than a standard alignment, quasi-mapping produces what we refer to as fragment mapping information. In particular, it provides, for each query (fragment), the reference sequences (transcripts), strand and position from which the query may have likely originated.
...
%		\includegraphics[width=\textwidth]{images/karma/karma_save.png}
...
\section{Results}
	...
	\subsection{Runtime analysis reveals \texttt{Linclust} as fastest clustering tool}
		...
		\begin{table}[H]
			\caption[Runtime of different clustering methods.]{Runtime (hh:mm:ss) of the different clustering methods for each dataset. The best runtime per sample is marked bold. }
			\renewrobustcmd{\bfseries}{\fontseries{b}\selectfont}
			\renewrobustcmd{\boldmath}{}
			\resizebox{\textwidth}{!}{%
		...
		\begin{itemize}
			...
			\item \textbf{\detonate} \citep{DETONATE:14}\\
				...
				\begin{itemize}
					...
					Reflects the similarity of the KC score compared to \detonate{}s estimated `true' assembly.					
...
\newpage
\section{Discussion}
	%This master's thesis aimed to improve a \textit{de novo} transcriptome assembly through a combination of multiple assemblers and to cluster the results to remove the redundancy. 
	...
	\subsection{Uneven sequence length distribution may be a challenge for \texttt{MeShClust}}
	    ...
	    %Additionally, the evaluation metrics don't seem to follow a certain pattern. For example, \mclust gets a relatively high score (0.91) for fragmented BUSCOs on the paired-end \ecoli dataset (Figure~\ref{img:eco_pe}). Yet, on the single-end \ecoli dataset (Figure~\ref{img:eco_se}) it achieves the lowest possible score (0). Contrarily, \cdhit (c=0.9) gets consistently high scores ($\geq0.97$) for RSEM EVAL and KC score for all datasets.
		...
		For the paired-end \celegans dataset \mclusttwo gets a score of 10.12 and \mclust gets a score of 8.91 (Figure~\ref{img:cel_pe}). \mclusttwo scores a 8.53 and \mclust 8.19 on the single-end \ecoli dataset (Figure~\ref{img:eco_se}).
...
\href{https://github.com/lmfaber/master_thesis/tree/master/supplemental_data/graphs}{https://github.com/lmfaber/master\_thesis/tree/master/supplemental\_data}\\
\textbf{Subfolders:}
\begin{itemize}
